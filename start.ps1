...
if (($startType -eq "all" -or $startType -eq "server") -and -not (Test-Path $serverExe)) {
    Write-ColorOutput "errors.executable_not_found" -ForegroundColor "Red" -Prefix "ОШИБКА" -FormatArgs @("server", $serverExe)
    exit 1
}
...
Write-ConfigParam "info.server_preset" $selectedServerPreset
Write-ConfigParam "info.mod_preset" $selectedModPreset
if ($isExperimental) {
...
if ($shouldClearLogs) {
    Write-ColorOutput "info.clearing_logs" -ForegroundColor "Yellow" -Prefix "ЛОГИ  "
...
if ((Test-Path $serverPath) -and (($startType -eq "all" -or $startType -eq "server") -or -not $startType)) {
    ...
    if ($mod) {
        Write-ColorOutput "info.client_mods" -ForegroundColor "Cyan" -Prefix "СИСТЕМА"
        $clientMods | ForEach-Object {
            Write-ColorOutput "info.list_item" -ForegroundColor "White" -Prefix "СИСТЕМА" -FormatArgs @($_)
        }
        Write-Host ""
    }

    if ($serverMod) {
        Write-ColorOutput "info.server_mods" -ForegroundColor "Cyan" -Prefix "СИСТЕМА"
    ...
    Write-ColorOutput "info.starting_server" -ForegroundColor "Green" -Prefix "СИСТЕМА" -FormatArgs @($serverPort)
...
if ((Test-Path $gamePath) -and (($startType -eq "all" -or $startType -eq "client") -or -not $startType)) {
    Write-ColorOutput "info.starting_client" -ForegroundColor "Green" -Prefix "СИСТЕМА" -FormatArgs @($serverPort)
